...
#define BLYNK_DEVICE_NAME "SafetyMeanU"
#define BLYNK_AUTH_TOKEN "c7Bhmn-pUTZSCfkWoUqGLjkWUaLUvRAQ"
const char* ssid = "somboon_5G"; // Enter your Wifi name 
const char* password = "88888888"; // Enter wifi password 
char auth[] = "Auth Token"; // Enter the Auth Token provied by Blynk 
...
//    /*CHECK NULL*/
    if(sender != NULL && sender != NULL && recipient != NULL && incomingMsgId != NULL && incomingLength != NULL && incoming != NULL){
      if(String(recipient, HEX) == "ff" && String(incomingLength)== "3" && incoming == "MEA"){
        ...
        Blynk.notify("Crash"); // This notification will be sent to Blynk App
        displayData(sender);
      }
    }
...
void sentNoti(){
  display.clearDisplay();
  display.setCursor(0,0);
   display.print("Send Notification to Blynk");
}
